...
public class FpbEvent
{
    ...
    public FpbEvent(List<string> cells, List<string> headers)
    {
    ...
    private void AddStartAndEndDate(List<string> cells, List<string> headers)
    {
        ...
        if (monthPos == -1 || dayPos == -1)
        {
            ...
            EndDate = DateTime.MinValue.ToString("yyyy-MM-dd");
            return;
        }
        ...
        StartDate = DateTime.ParseExact($"{startDay}-{startMonth}-{year}", "dd-MM-yyyy", null).ToString("yyyy-MM-dd");
        EndDate = DateTime.ParseExact($"{endDay}-{month}-{year}", "dd-MM-yyyy", null).ToString("yyyy-MM-dd");
    }
